...
func isinSeriesType(k reflect.Kind) bool {
...
func resolveDataFromRows(rows Rowss) ([]map[string]interface{}, error) {
	if nil == rows {
	...
	for rows.Next() {
		...
		for idx := range columns {
			...
			if columns[idx].DatabaseTypeName() == "UNIQUEIDENTIFIER" {
				mp[name] = uniqueIdentifiervalue(v.([]byte))
			} else if columns[idx].DatabaseTypeName() == "BIT" {
				if v.(bool) == true {
					mp[name] = 1
				} else {
					mp[name] = 0
...
func uniqueIdentifiervalue(u []byte) (s string) {
	reverse := func(b []byte) {
	...
	reverse(raw[4:6])
	reverse(raw[6:8])
	s = fmt.Sprintf("%X-%X-%X-%X-%X", raw[0:4], raw[4:6], raw[6:8], raw[8:10], raw[10:])
	return
}
