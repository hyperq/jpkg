...
import (
	...
	"github.com/hyperq/jpkg/tool/util"
)
...
func (c *Client) Type(typeStr RequestType) (client *Client) {
	if _, ok := types[typeStr]; ok {
		...
		c.Errors = append(c.Errors, errors.New("InvoiceType func: incorrect type \""+string(typeStr)+"\""))
	}
	return c
}
...
func (c *Client) EndStruct(v interface{}) (res *http.Response, errs []error) {
	...
	case string(TypeXML):
		...
		return res, nil
	default:
		c.Errors = append(c.Errors, errors.New("unmarshalType InvoiceType Wrong"))
		return nil, c.Errors
	}
}
	...
	req, err := func() (*http.Request, error) {
		...
		case GET:
			...
			case TypeForm, TypeFormData, TypeUrlencoded:
				c.ContentType = types[TypeUrlencoded]
			case TypeMultipartFormData:
